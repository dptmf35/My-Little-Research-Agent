...
- ❓ 이 논문이 해결하려는 문제
- ⭐ 읽을 가치 평가 (1-10점) + 이유
- 📑 섹션 구조: 논문의 실제 섹션 헤더를 번호 포함하여 순서대로 나열하세요.
  반드시 아래 형식으로 작성하세요:
  [SECTIONS]: ["1. Introduction", "2. Related Work", "3. Method", ...]
...
INTEGRATED_PART_C_DYNAMIC = (_INTEGRATED_BASE + """
아래는 논문의 실제 섹션 목록입니다. 이 섹션들을 논문 원본 이름 그대로 ## 헤딩으로 사용하여 순서대로 상세 분석하세요.

섹션 목록:
{{section_list}}

각 섹션에 대해:
- 해당 섹션의 핵심 내용을 상세히 설명
- 핵심 수식이 있다면 $$...$$ 형식으로 정확히 기술하고 변수 의미 설명
- 알고리즘이 있다면 단계별 설명 또는 ```text ... ``` 블록으로 표현
- 실험 결과가 있다면 주요 수치와 비교 분석
{{figure_section}}""").format(format_rules=_FORMAT_RULES_INTEGRATED).replace("{{pass1_result}}", "{pass1_result}").replace("{{pass2_result}}", "{pass2_result}").replace("{{pass3_result}}", "{pass3_result}").replace("{{section_list}}", "{section_list}").replace("{{figure_section}}", "{figure_section}")

INTEGRATED_PART_C_FALLBACK = (_INTEGRATED_BASE + """
...
{{figure_section}}""").format(format_rules=_FORMAT_RULES_INTEGRATED).replace("{{pass1_result}}", "{pass1_result}").replace("{{pass2_result}}", "{pass2_result}").replace("{{pass3_result}}", "{pass3_result}").replace("{{figure_section}}", "{figure_section}")
...
def _extract_section_list(pass1_result: str) -> list:
    """
    Pass 1 결과에서 [SECTIONS]: [...] 형식의 섹션 목록을 파싱한다.
    실패 시 빈 리스트를 반환 (graceful fallback).
    """
    pattern = r'\[SECTIONS\]\s*:\s*\[([^\]]*)\]'
    match = re.search(pattern, pass1_result)
    if not match:
        return []
    try:
...
def analyze_paper(paper_data: dict, progress_callback=None) -> dict:
    ...
    # Figure 조건부: figures가 있으면 Figure 분석 섹션을 Part C에 삽입
